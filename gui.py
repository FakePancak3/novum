...
gui.py
------
The presentation layer. Deliberately minimal: plain typography, a lot
of negative space, underline-style inputs instead of boxed cards, and
one accent color used sparingly. All actual clicking/timing logic
lives in engine.py; all hotkey logic lives in keybind_manager.py — this
module only wires UI events to those two and renders state changes.
"""
...
# Name of the icon file, expected to sit next to this script during
# development and to be bundled into the exe via PyInstaller's
# --add-data flag for the packaged build.
ICON_FILENAME = "icon.ico"


def _resolve_icon_path():
    """Find icon.ico whether running from source or from a PyInstaller
    --onefile exe (which extracts bundled data to a temp folder exposed
    as sys._MEIPASS at runtime)."""
...
class RoundedButton(tk.Canvas):
    """A rounded, hover/press-responsive button drawn on a Canvas."""
...
class MinimalEntry(tk.Frame):
    """A flat, borderless numeric input with just a bottom rule — no
    filled box — that highlights on focus. Reads as plain typography
    rather than a UI "card"."""
    ...
    def __init__(self, parent, width, initial="", font=theme.FONT_INPUT, justify="center"):
        ...
        # Keep the underline's drawn rectangle matching the canvas's
        # actual (stretched) width, so it lines up with the entry above
        # it instead of staying fixed at the initial `width` argument.
        self._line.bind(
            "<Configure>",
...
class NovumApp:
    """Top-level application: builds the window and wires UI <-> engine."""
    ...
    def _configure_root(self):
        ...
        # Explicitly set the window/taskbar icon. Tkinter draws its own
        # default icon on every window (overriding whatever icon the
        # exe itself was built with), so this is required in addition
        # to PyInstaller's --icon flag — that flag only controls how
        # the .exe file looks in File Explorer, not the live window.
    ...
    def _build_ui(self):
        ...
        # Title — plain, letter-spaced, no subtitle/logo flourish.
        tk.Label(
            outer, text="N O V U M", bg=theme.BG, fg=theme.TEXT_PRIMARY,
            font=self.font_title,
        ).pack(anchor="w")

        # CPS inputs — two plain underline fields side by side.
        ...
        # Inline error text (plain, no box) — hidden until needed.
        ...
        # Footer — plain text on both sides, no chips/boxes.
